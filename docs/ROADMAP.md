...
جدول وضعیت بالا و نمودار وابستگی در پایان فیچر 002 به‌روز شد. قبلاً ردیف 002
به‌اشتباه همچنان «not-started» نشان داده می‌شد (drift ایجادشده در فاز plan، که
قبل از شروع implement نوشته شده بود و دیگران فراموش کردند در پایان به‌روزش کنند).
این به‌تنهایی وضعیت 002 را درست کرد تا با `PROJECT_STATUS.md`،
`specs/002/status.md`، و git log هم‌خوان باشد. بقیهٔ محتوای narrative این فایل
(دلیل انتخاب 002) به‌عنوان سابقهٔ تصمیم دست‌نخورده باقی ماند.
