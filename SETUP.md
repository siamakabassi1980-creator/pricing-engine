...
> SETUP.md قبلاً `../shared-kit/` می‌گفت، ولی ساختار واقعی این است که
> `pricing-engine` و `shared-kit` sibling نیستند — مسیر درست
> `../ZCodeProject/shared-kit/` است. هر مسیر نسبی در template را با `ls`
> پیش از اعمال تأیید کن.

## نصب pre-commit

```bash
./venv/Scripts/python.exe -m pip install pre-commit
cp ../ZCodeProject/shared-kit/.pre-commit-config.yaml .
./venv/Scripts/python.exe -m pip install pre_commit autoupdate
...
```bash
cp ../ZCodeProject/shared-kit/AGENTS.md .                # نازک — فقط ارجاع به constitution
cp ../ZCodeProject/shared-kit/.gitignore .
cp ../ZCodeProject/shared-kit/.pre-commit-config.yaml .
# constitution-template.md به‌عنوان نقطهٔ شروع برای .specify/memory/constitution.md
cp ../ZCodeProject/shared-kit/constitution-template.md .specify/memory/constitution.md
```
